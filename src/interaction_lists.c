...
void pc_compute_interaction_list(int tree_numnodes, const int *tree_level, 
                ...
                double batch_x_mid, double batch_y_mid, double batch_z_mid, double batch_radius,
...
{
    int batch_numnodes;
    const int *batch_numpar;
    const double *batch_x_mid, *batch_y_mid, *batch_z_mid, *batch_radius;
    int *batch_num_direct, *batch_num_approx;
    
    int tree_numnodes;
    const int *tree_numpar, *tree_level;
    const double *tree_x_mid, *tree_y_mid, *tree_z_mid, *tree_radius;

    batch_numnodes = batches->numnodes;
    batch_numpar = batches->numpar;
    batch_x_mid = tree_array->x_mid;
    batch_y_mid = tree_array->y_mid;
    batch_z_mid = tree_array->z_mid;
    batch_radius = batches->radius;

    batch_num_direct = batches->numDirect;
    batch_num_approx = batches->numApprox;
    ...
    for (int i = 0; i < batches->numnodes; i++)
        pc_compute_interaction_list(tree_numnodes, tree_level, tree_numpar,
                ...
                batch_x_mid[i], batch_y_mid[i], batch_z_mid[i], batch_radius[i],
                ...
                int *batch_tree_list, int *batch_direct_list)
{
    ...
    for (j = 0; j < tree_numnodes; j++) {
        if (tree_level[j] <= current_level) {
            ...
            tx = batch_x_mid - tree_x_mid[j];
            ty = batch_y_mid - tree_y_mid[j];
            tz = batch_z_mid - tree_z_mid[j];
        ...
        if (tree_num_children[tree_node] == 0) {
            ...
            for (int i = 0; i < tree_num_children[tree_node]; i++) {
                pc_compute_interaction_list_remote(tree_children[8*tree_node + i],
                           ...
                           batch_x_mid, batch_y_mid, batch_z_mid, batch_radius,
