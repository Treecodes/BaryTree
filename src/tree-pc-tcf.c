...
void pc_interaction_list_treecode_tcf(struct tnode_array *tree_array,
...
//			printf("\n\nInside compute region, clusters->q[213599] = %f\n\n",clusters->q[213599]);
	...
	#pragma acc data copyin(xS[0:sources->num], yS[0:sources->num], zS[0:sources->num], qS[0:sources->num], wS[0:sources->num], \
        ...
        double kap_eta_2 = kappa * eta / 2.0;
		...
		int streamID;
		#pragma omp for private(j,ii,jj,batch_ibeg,batch_iend,numberOfClusterApproximations,numberOfDirectSums,numberOfTargets,batchStart,node_index,clusterStart,streamID,kap_r,r_eta)
			...
			for (j = 0; j < numberOfClusterApproximations; j++) {
				...
				for (ii = 0; ii < numberOfTargets; ii++){
					...
					for (jj = 0; jj < numberOfInterpolationPoints; jj++){
                        ...
                        kap_r = kappa * r;
                        r_eta = r / eta;
						tempPotential += qC[clusterStart + jj] / r
