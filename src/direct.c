                ...
                int pot_type, float kappa)
{
    /* local variables */
    int i, j;
    float tx, ty, tz, xi, yi, zi, qi, teng, rad;
    ...
    if (pot_type == 0 || pot_type == 4) { // Coulomb with singularity skipping.  Lagrange or Hermite.
        ...
        for (i = 0; i < numparsT; i++) {
            ...
            for (j = 0; j < numparsS; j++) {
                ...
                ty = yi - yS[j];
                tz = zi - zS[j];
                rad = sqrtf(tx*tx + ty*ty + tz*tz);
                if (rad > FLT_MIN) {
                    teng = teng + qS[j] * wS[j] * expf(-kappa * rad) / rad;
                }
            }
            denergy[i] += teng;
        }
        ...
        for (i = 0; i < numparsT; i++) {
            ...
            for (j = 0; j < numparsS; j++) {
                ...
                rad = sqrtf(tx*tx + ty*ty + tz*tz);
                if (rad > 1e-14) {
                    teng = teng + ( qS[j] - qi * expf(-rad*rad/kappaSq)) * wS[j] / rad;
                }
            }
            denergy[i] += teng;
        }
        ...
        for (i = 0; i < numparsT; i++) {
            ...
            for (j = 0; j < numparsS; j++) {
                ...
                rad = sqrtf(tx*tx + ty*ty + tz*tz);
                if (rad > 1e-14) {
                    teng += ( qS[j] - qi) * wS[j] * expf(-kappa * rad) / rad;
                }
            }
            denergy[i] += teng;
        }
...
//    printf("elements of denergy: %f, %f\n", denergy[0], denergy[1]);
