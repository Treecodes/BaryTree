...
void fill_in_cluster_data_hermite(struct particles *clusters, struct particles *sources, struct tnode *troot, int order)
{
    ...
    #pragma acc data copy(tt[0:torderlim],ww[0:torderlim], \
        sources->x[0:sources->num], sources->y[0:sources->num], sources->z[0:sources->num], \
        sources->q[0:sources->num], sources->w[0:sources->num], \
        clusters->x[0:clusters->num],   clusters->y[0:clusters->num],    clusters->z[0:clusters->num], \
        clusters->q[0:clusters->num],   clusters->qx[0:clusters->num],   clusters->qy[0:clusters->num], \
        clusters->qz[0:clusters->num],  clusters->qxy[0:clusters->num],  clusters->qyz[0:clusters->num], \
        clusters->qxz[0:clusters->num], clusters->qxyz[0:clusters->num], \
        clusters->w[0:clusters->num],   clusters->wx[0:clusters->num],   clusters->wy[0:clusters->num], \
        clusters->wz[0:clusters->num],  clusters->wxy[0:clusters->num],  clusters->wyz[0:clusters->num], \
...
#pragma acc kernels present(xS, yS, zS, qS, wS, clusterX, clusterY, clusterZ,tt,ww, \
    ...
    double numerator0, numerator1, numerator2, numerator3;
    double numerator4, numerator5, numerator6, numerator7;
    double xn, yn, zn, Ax, Ay, Az, Bx, By, Bz;
    double temp0, temp1, temp2, temp3, temp4, temp5, temp6, temp7;
    int k1, k2, k3, kk, sourcePointIndex, interpolationPointIndex;
    double cx, cy, cz;

    #pragma acc loop independent
    for (int j = 0; j < pointsPerCluster; j++) { // loop over interpolation points, set (dx,dy,dz) for this point
...
void addNodeToArray_hermite(struct tnode *p, struct particles *sources, struct particles *clusters, 
                            int order, int numInterpPoints, int pointsPerCluster)
{
