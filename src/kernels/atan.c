...
void atanDirect(int number_of_targets_in_batch, int number_of_source_points_in_cluster,
        ...
        for (int j = 0; j < number_of_source_points_in_cluster; j++) {
            ...
            if (fabs(dz-0.5) > DBL_MIN) {
                if (fabs(dz+0.5) > DBL_MIN) {
                    ...
                    modz += (0.5 - dz < 0 ? 1.0 : 0);
                    temporary_potential += source_charge[jj] * source_weight[jj] * ( 1/M_PI * atan( sqrt( 1 + 1.0/(delta*delta))* tan(M_PI * dz)) + modz - 0.5);
                }
            }
        ...
        struct kernel *kernel, double *potential, int gpu_async_stream_id)
{
    ...
    for (int i = 0; i < number_of_targets_in_batch; i++) {
        ...
        for (int j = 0; j < number_of_interpolation_points_in_cluster; j++) {
            ...
            if (fabs(dz-0.5) > DBL_MIN) {
                if (fabs(dz+0.5) > DBL_MIN) {
//                    temporary_potential += cluster_charge[starting_index_of_cluster + j] * ( 1/M_PI * atan( sqrt( 1 + 1./(delta*delta))* tan(M_PI * dz)) + modz - .5);
                }
            }
