...
//                                + 15 * r7inv *   cluster_delta_xyz[jj]*dx*dy*dz;
                ...
                temporary_potential +=       exp(-kernel_parameter*r)*(
                            ...
                            +      r3inv * (1 + kernel_parameter*r) * ( cluster_delta_x[jj]*dx +  cluster_delta_y[jj]*dy +  cluster_delta_z[jj]*dz )
                            +      r5inv * (3 + 3*kernel_parameter*r + kernel_parameter2*r2 ) * ( cluster_delta_xy[jj]*dx*dy +  cluster_delta_yz[jj]*dy*dz +  cluster_delta_xz[jj]*dx*dz )
                            +      r7inv * (15 + 15*kernel_parameter*r + 6*kernel_parameter2*r2 + kernel_parameter3*r3) * cluster_delta_xyz[jj]*dx*dy*dz);
