...
static void pc_comp_ms_modifiedF_SS_child_to_parent_Q(const struct Tree *tree, int child_index, int parent_index, int interpolationDegree,
...
static void pc_comp_ms_modifiedF_SS_W(const struct Tree *tree, int idx, int interpolationDegree,
                double *xS, double *yS, double *zS, double *qS, double *wS,
                double *clusterX, double *clusterY, double *clusterZ, double *clusterQ, double *clusterW);
...
/*
        ...
        for (int i = 0; i < tree->leaves_list_num; ++i) {
                int leaf_index = tree->leaves_list[i];
                pc_comp_ms_modifiedF_SS_Q(tree, leaf_index, interpolationDegree, xS, yS, zS, qS, wS, xC, yC, zC, qC, wC);
        }

        // interpolate up clusters, level by level
        for (int level = tree->max_depth-1; level >= 0; --level) {
            printf("anterpolating Q at level %i.\n", level);
            for (int cluster_index = 0; cluster_index < tree->levels_list_num[level]; ++cluster_index) {
                ...
                for (int child_counter=0; child_counter<tree->num_children[parent_index]; ++child_counter){
                    ...
                    pc_comp_ms_modifiedF_SS_child_to_parent_Q(tree, child_index, parent_index, interpolationDegree, xC, yC, zC, qC, wC);
        ...
        // anterpolate from particles to leaf cluster interpolation points
        printf("anterpolating W to leaves.\n");
        for (int i = 0; i < tree->leaves_list_num; ++i) {
                int leaf_index = tree->leaves_list[i];
                pc_comp_ms_modifiedF_SS_W(tree, leaf_index, interpolationDegree, xS, yS, zS, qS, wS, xC, yC, zC, qC, wC);
        }

        // interpolate up clusters, level by level
        for (int level = tree->max_depth-1; level >= 0; --level) {
            printf("anterpolating W at level %i.\n", level);
            for (int cluster_index = 0; cluster_index < tree->levels_list_num[level]; ++cluster_index) {
                ...
                for (int child_counter=0; child_counter<tree->num_children[parent_index]; ++child_counter){
                    ...
                    pc_comp_ms_modifiedF_SS_child_to_parent_W(tree, child_index, parent_index, interpolationDegree, xC, yC, zC, qC, wC);
...
void pc_comp_ms_modifiedF_SS_child_to_parent_Q(const struct Tree *tree, int child_index, int parent_index, int interpolationDegree,
        double *clusterX, double *clusterY, double *clusterZ, double *clusterQ, double *clusterW)
{
...
void pc_comp_ms_modifiedF_SS_child_to_parent_W(const struct Tree *tree, int child_index, int parent_index, int interpolationDegree,
        double *clusterX, double *clusterY, double *clusterZ, double *clusterQ, double *clusterW)
{
...
void pc_comp_ms_modifiedF_SS_W(const struct Tree *tree, int idx, int interpolationDegree,
        double *xS, double *yS, double *zS, double *qS, double *wS,
        double *clusterX, double *clusterY, double *clusterZ, double *clusterQ, double *clusterW)
{
    int interpDegreeLim = interpolationDegree + 1;
