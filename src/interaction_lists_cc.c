...
void cc_compute_interaction_list(int tree_numnodes, const int *tree_level, 
...
void cc_compute_interaction_list_remote(int tree_node, const int *tree_numpar, const double *tree_radius,
                const double *tree_x_mid, const double *tree_y_mid, const double *tree_z_mid,
...
void Interaction_CC_MakeList(const struct tnode_array *tree_array, struct tnode_array *batches,
...
void Interaction_CC_MakeListRemote(const struct tnode_array *source_tree_array, struct tnode_array *target_tree_array,
                                   int *approx_list_unpacked,int *approx_list_packed, int *direct_list,
                                   int *num_target_approx, int *num_target_direct,
                                   int interpolationOrder, double sizeCheckFactor)
{
    int target_tree_numnodes = target_tree_array->numnodes;
    const int *batch_numpar = target_tree_array->numpar;
    ...
    int *target_num_direct = target_tree_array->numDirect;
    int *target_num_approx = target_tree_array->numApprox;

    int tree_numnodes = source_tree_array->numnodes;
    const int *tree_numpar = source_tree_array->numpar;
    const int *tree_level = source_tree_array->level;
    const double *tree_radius = source_tree_array->radius;
    const double *tree_x_mid = source_tree_array->x_mid;
    const double *tree_y_mid = source_tree_array->y_mid;
    const double *tree_z_mid = source_tree_array->z_mid;

    const int *tree_num_children = source_tree_array->num_children;
    const int *tree_children = source_tree_array->children;
    ...
    make_matrix(temp_tree_inter_list, target_tree_array->numnodes, 50);
    make_matrix(temp_direct_inter_list, target_tree_array->numnodes, 50);

    make_vector(num_tree_inter, target_tree_array->numnodes);
    make_vector(num_direct_inter, target_tree_array->numnodes);

    make_vector(sizeof_tree_inter_list, target_tree_array->numnodes);
    make_vector(sizeof_direct_inter_list, target_tree_array->numnodes);
   
    int loopsize = target_tree_array->numnodes;
    ...
    // !!!!!!!!!!!!!!!!!!!!
    for (int i = 0; i < target_tree_array->numnodes; i++) {
...
void cc_compute_interaction_list_1_remote(int source_tree_node, const int *source_tree_numpar, const double *source_tree_radius,
                ...
                int *target_num_direct, int *target_num_approx,
 ...
 *                */
        ...
        if (*tree_index_counter >= *sizeof_tree_list) {
            ...
            (*target_tree_list) = realloc_vector(*target_tree_list, *sizeof_tree_list);
        }

        (*target_tree_list)[*tree_index_counter] = source_tree_node;
        (*tree_index_counter)++;
 ...
 *                */
        if (target_tree_num_children[target_tree_node] == 0) {
            ...
            if (*direct_index_counter >= *sizeof_direct_list) {
                ...
                *target_direct_list = realloc_vector(*target_direct_list, *sizeof_direct_list);
            }

            (*target_direct_list)[*direct_index_counter] = tree_node; 
            (*direct_index_counter)++;
