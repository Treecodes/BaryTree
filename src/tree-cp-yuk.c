                     ...
                     int arrdim, double kappa)
{
    ...
    //    printf("        distsq: %f\n", distsq);
         ...
         */
                if (p->num_children == 0) {
                        comp_direct_yuk(EnP, p->ibeg, p->iend,
                                    x, y, z, arrdim, kappa);
                } else {
                        for (i = 0; i < p->num_children; i++) {
                                compute_cp1_yuk(p->child[i], EnP,
                                            x, y, z, arrdim, kappa);
                        }
                }
        }
...
{
        /* local variables */
        double tdx, tdy, tdz, fac, sqfac, t1, dist, dist2;
